  ...
  async function loadData() {
    ...
    document.querySelector("#d-name").value = data[0].name;
  }
  
  loadData();
document.querySelector("#save-name").addEventListener("click", async () => {
  const { data: existing } = await supabase.from('portfolio').select('*');

  if (existing.length === 0) {
    await supabase.from('portfolio').insert([{
      name: document.querySelector("#d-name").value
    }]);
  } else {
    await supabase.from('portfolio')
      .update({ name: document.querySelector("#d-name").value })
      .eq('id', existing[0].id);
  }
